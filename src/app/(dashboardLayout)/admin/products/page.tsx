...
const ProductManagement = () => {
  ...
  useEffect(() => {
    setPage(1);
  }, [filters.searchTerm, filters.status, filters.flashSale]);
  ...
  return (
    <div className="flex flex-col min-h-screen p-2 space-y-6">
      {/* Header Section */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground ps-5">
            Manage and monitor all products in your system
          </p>
        </div>
      ...
      <div className="flex-grow border border-slate-200/60 rounded-none shadow-xl p-2 min-h-screen">
        {products.length === 0 ? (
          ...
          <ProductTable
            ...
            onDelete={(product) => handleDeleteProduct(product.id)}
            onUpdateStatus={handleUpdateStatusProduct}
            onView={(product) => router.push(`/admin/products/${product.id}`)}
